...
from models import Users, Key, KeyHistory, TransferRequest
...
def get_key_history():
    try:
        history_records = KeyHistory.query.order_by(KeyHistory.created_at.desc()).all()
        history_list = []
        for record in history_records:
            ...
            if key and user:
                history_list.append({
                    ...
                    "timestamp": record.created_at.strftime("%d.%m.%Y %H:%M")
                })
        ...
        for key_obj in issued_keys:
            ...
            if last_history and last_history.user_id == user_id:
                keys_list.append({
                    ...
                    "last_user": last_history.user.fio if last_history.user else None,
...
def request_key():
    ...
    if key_obj.category not in user_categories:
        return jsonify({"status": "error", "message": "У вас нет прав на этот ключ"}), 403
    if not key_obj.status:
        return jsonify({"status": "error", "message": "Ключ уже выдан"}), 400
    new_hist = KeyHistory(user_id=user_id, key_id=key_id, action="request")
